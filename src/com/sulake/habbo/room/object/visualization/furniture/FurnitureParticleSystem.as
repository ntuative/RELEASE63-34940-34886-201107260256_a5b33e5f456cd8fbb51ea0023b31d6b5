package com.sulake.habbo.room.object.visualization.furniture
{
   ...
   public class FurnitureParticleSystem
   {
      ...
      private var var_877:ColorTransform;
      
      private var var_1632:ColorTransform;
      
      private var var_2096:Matrix;
      ...
      public function FurnitureParticleSystem(param1:AnimatedFurnitureVisualization)
      {
         ...
         this.var_877 = new ColorTransform();
         this.var_877.alphaMultiplier = 1;
         this.var_1632 = new ColorTransform();
         this.var_2096 = new Matrix();
      ...
      public function setAnimation(param1:int) : void
      {
         ...
         this.var_114 = this.var_1334[param1];
         this.var_1066 = false;
         this.updateCanvas();
      }
      ...
      private function updateCanvas() : void
      {
         ...
         if(this.var_2095 >= 0)
         {
            this.var_298 = this._visualization.getSprite(this.var_2095);
            if(this.var_298 && this.var_298.asset)
            {
               ...
               if(this.var_98 == null)
               {
                  this.var_98 = this.var_298.asset.clone();
                  if(this.var_877.alphaMultiplier != 1)
                  {
                     this.var_707 = new BitmapData(this.var_98.width,this.var_98.height,true,4278190080);
                  }
               }
               this.var_2818 = -this.var_298.offsetX;
               this.var_2817 = -this.var_298.offsetY;
               this.var_298.asset = this.var_98;
            }
            if(this.var_98)
            {
            ...
            if(this.var_707)
            {
               this.var_707.fillRect(this.var_707.rect,4278190080);
            }
         }
      }
      ...
      public function getSpriteYOffset(param1:int, param2:int, param3:int) : int
      {
         ...
         {
            return this.var_114.y * this.var_1333;
         }
         return 0;
      }
      ...
      {
         if(!this.var_114 || !this.var_298)
         {
            return;
         }
         if(this.var_98 && this.var_298.asset != this.var_98)
         {
            this.var_298.asset = this.var_98;
         }
         if(this.var_1066)
      ...
      public function updateAnimation() : void
      {
         ...
         var _loc2_:* = 0;
         var _loc8_:int = 0;
         if(!this.var_1066 && this.var_114.hasIgnited)
         {
            this.var_1066 = true;
         }
         _loc8_ = this.var_1283 * this.var_1333;
         this.var_114.update();
         if(this.var_1066)
         {
            if(this.var_114.roomObjectSpriteId >= 0)
            ...
            {
               this.var_98.draw(this.var_707,this.var_2096,this.var_877,BlendMode.NORMAL,null,false);
            }
            for each(_loc11_ in this.var_114.particles)
            {
               _loc2_ = Number(_loc11_.y);
               _loc3_ = this.var_2818 + (_loc11_.x - _loc11_.z) * 10 / 10 * this.var_1333;
               _loc4_ = this.var_2817 - _loc8_ + (_loc2_ + (_loc11_.x + _loc11_.z) / 2) * 10 / 10 * this.var_1333;
               ...
               if(_loc6_)
               {
                  ...
                  if(_loc11_.fade && _loc11_.alphaMultiplier < 1)
                  {
                     ...
                     this.var_1632.alphaMultiplier = _loc11_.alphaMultiplier;
                     this.var_98.draw(_loc7_,this._translationMatrix,this.var_1632,BlendMode.NORMAL,null,false);
                  }
                  else
                  {
      ...
      public function parseData(param1:XML) : void
      {
         ...
         var _loc23_:* = null;
         var _loc24_:* = null;
         this.var_2871 = parseInt(param1.@size);
         this.var_2095 = !!param1.hasOwnProperty("@canvas_id") ? int(parseInt(param1.@canvas_id)) : -1;
         this.var_1283 = !!param1.hasOwnProperty("@offset_y") ? int(parseInt(param1.@offset_y)) : 10;
         this.var_1333 = this.var_2871 / 64;
         this._blend = !!param1.hasOwnProperty("@blend") ? Number(Number(param1.@blend)) : Number(1);
         this._blend = Math.min(this._blend,1);
         this.var_877.alphaMultiplier = this._blend;
