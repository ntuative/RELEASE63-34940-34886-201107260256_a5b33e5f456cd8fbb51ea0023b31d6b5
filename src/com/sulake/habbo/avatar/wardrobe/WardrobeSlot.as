package com.sulake.habbo.avatar.wardrobe
{
   ...
   public class WardrobeSlot implements IOutfit, IAvatarImageListener
   {
      ...
      private var var_392:IBitmapWrapperWindow;
      ...
      private function createView() : void
      {
         ...
         this.var_392 = this._view.findChildByName("image") as IBitmapWrapperWindow;
      }
      ...
      public function updateView() : void
      {
         ...
         if(this.var_667 && this.var_1544)
         {
            _loc5_ = this.var_27.manager.avatarRenderManager.createAvatarImage(this.figure,AvatarScaleType.const_113,this.var_1016,this);
            if(_loc5_)
            {
               _loc5_.setDirection(AvatarSetType.const_43,parseInt(FigureData.const_951));
         ...
         if(this.var_392)
         {
            if(this.var_392.bitmap)
            {
               this.var_392.bitmap.dispose();
            }
            this.var_392.bitmap = new BitmapData(this.var_392.width,this.var_392.height,true,0);
            _loc7_ = (this.var_392.width - _loc1_.width) / 2;
            _loc8_ = (this.var_392.height - _loc1_.height) / 2;
            this.var_392.bitmap.draw(_loc1_,new Matrix(1,0,0,1,_loc7_,_loc8_));
         }
         if(_loc2_)
         {
            _loc1_.dispose();
      ...
      private function eventHandler(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         {
            case "set_button":
               this.var_667 = this.var_27.figureData.getFigureString();
               this.var_1016 = this.var_27.gender;
               this.var_27.handler.saveWardrobeOutfit(this.var_1941,this);
               this.updateView();
               break;
            case "get_button":
               if(this.var_667)
               {
                  this.var_27.loadAvatarInEditor(this.var_667,this.var_1016,this.var_27.clubMemberLevel);
               }
         }
      }
