package com.sulake.habbo.avatar.common
{
   ...
   public class AvatarEditorGridPartItem implements IAvatarImageListener
   {
      ...
      private var var_790:IFigurePartSet;
      ...
      private const const_2380:int = 13421772;
      
      private const const_2381:int = 16777215;
      
      private const const_388:Array = [2,6,0,4,3,1];
      ...
      public function AvatarEditorGridPartItem(param1:IWindowContainer, param2:IAvatarEditorCategoryModel, param3:IFigurePartSet, param4:Array, param5:Boolean = true)
      {
         ...
         this.var_790 = param3;
         this._window = param1;
         this.var_1495 = this._window.findChildByTag("BG_COLOR");
         this._colors = param4;
         ...
         if(param3 != null)
         {
            ...
            {
               this.var_1844 = Math.max(this.var_1844,_loc6_.colorLayerIndex);
            }
         }
         this.var_444 = this.var_78.controller.manager.avatarRenderManager;
      ...
      private function updateThumbVisualization() : void
      {
         ...
         if(_loc1_)
         {
            if(this.var_753 != null && !this.var_1843)
            {
               _loc3_ = this.var_753;
            }
            else
         ...
         if(this.isSelected)
         {
            this.var_1495.color = this.const_2380;
         }
         else
         {
            this.var_1495.color = this.const_2381;
         }
      ...
      private function analyzePartLayers() : Boolean
      {
         ...
         for each(_loc2_ in this.partSet.parts)
         {
            if(_loc7_)
            {
               _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_388[_loc6_] + "_" + FigureData.const_150;
               _loc4_ = this.var_444.getAssetByName(_loc3_) as BitmapDataAsset;
            }
            else
            {
               _loc6_ = 0;
               while(!_loc7_ && _loc6_ < this.const_388.length)
               {
                  _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_388[_loc6_] + "_" + FigureData.const_150;
                  _loc4_ = this.var_444.getAssetByName(_loc3_) as BitmapDataAsset;
      ...
      private function renderThumb() : BitmapData
      {
         ...
         _loc1_ = new BitmapData(this.var_645.width,this.var_645.height,true,16777215);
         var _loc6_:int = 0;
         var _loc7_:Boolean = false;
         ...
         for each(_loc2_ in _loc8_)
         {
            ...
            if(_loc4_)
            {
               ...
               _loc10_ = -1 * _loc4_.offset.x - this.var_645.x;
               _loc11_ = -1 * _loc4_.offset.y - this.var_645.y;
               if(this.var_1843 && _loc2_.colorLayerIndex > 0)
               {
