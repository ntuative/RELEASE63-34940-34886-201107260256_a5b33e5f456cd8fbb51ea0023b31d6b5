package com.sulake.habbo.roomevents.userdefinedroomevents.help
{
   ...
   public class UserDefinedRoomEventsHelp
   {
      ...
      private function prepareWindow() : void
      {
         ...
         this.var_2209 = IWindowContainer(this.find(this._window,"help_container"));
         this.var_1409 = this.var_2209.findChildByName("button_previous") as IContainerButtonWindow;
         if(this.var_1409)
         {
            this.var_1409.procedure = this.onButtonClick;
         }
         this.var_1408 = this.var_2209.findChildByName("button_next") as IContainerButtonWindow;
         if(this.var_1408)
         {
            this.var_1408.procedure = this.onButtonClick;
         }
      ...
      {
         if(this.var_312 >= this.var_74.length - 1)
         {
            this.var_1408.disable();
         }
         else
         {
            this.var_1408.enable();
         }
         if(this.var_312 <= 0)
         {
            this.var_1409.disable();
         }
         else
         {
            this.var_1409.enable();
         }
      }
      ...
      {
         ++this.var_312;
         this.var_312 = Math.min(this.var_312,this.var_74.length - 1);
         this.openPage();
      }
      ...
      {
         --this.var_312;
         this.var_312 = Math.max(this.var_312,0);
         this.openPage();
      }
      ...
      private function localize() : void
      {
         ...
         if(_loc1_)
         {
            _loc1_.text = ILocalization(this.var_74[this.var_312][0]).value;
         }
         if(_loc2_)
         {
            _loc2_.text = ILocalization(this.var_74[this.var_312][2]).value;
         }
         if(_loc3_)
         {
            _loc4_ = ILocalization(this.var_74[this.var_312][1]).value;
