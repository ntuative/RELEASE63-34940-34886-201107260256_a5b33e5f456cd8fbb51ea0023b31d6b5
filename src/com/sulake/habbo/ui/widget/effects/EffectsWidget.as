package com.sulake.habbo.ui.widget.effects
{
   ...
   public class EffectsWidget extends RoomWidgetBase
   {
      ...
      private static const const_1484:int = 320;
      
      private static const const_1486:int = 48;
      ...
      public function update() : void
      {
         ...
         {
            _loc1_ = this.var_373.getValue(_loc3_.type) as EffectView;
            if(_loc1_)
            {
               _loc1_.update();
            }
            else
            {
               ...
               this.var_373.add(_loc3_.type,_loc1_);
               this.var_64.addListItem(_loc1_.window);
            }
         }
         _loc4_ = this.var_373.length - 1;
         while(_loc4_ >= 0)
         {
            _loc1_ = this.var_373.getWithIndex(_loc4_);
            if(_loc2_.indexOf(_loc1_.effect) == -1)
            {
               this.var_64.removeListItem(_loc1_.window);
               this.var_373.remove(this.var_373.getKey(_loc4_));
               _loc1_.dispose();
            }
            _loc4_--;
         }
         var _loc5_:int = this.var_64.scrollableRegion.height;
         this.var_64.height = Math.max(Math.min(_loc5_,const_1484),const_1486);
      ...
      {
         this.handler.container.catalog.openCatalogPage(CatalogPageName.const_1230,true);
      }
   }
}
