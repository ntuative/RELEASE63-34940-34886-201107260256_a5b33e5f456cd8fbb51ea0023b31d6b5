package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class PetsCatalogWidget extends CatalogWidget implements ICatalogWidget, IPetDataListener, IPetImageListener
   {
      ...
      override public function init() : Boolean
      {
         ...
         this.var_573 = this.getPetTypeIndexFromProduct(_loc3_.localizationId);
         if(this.var_573 >= 8)
         {
            return false;
         }
         this.updateAvailableBreeds(_loc3_.localizationId);
         this.var_2854 = _loc3_.localizationId;
         this.var_428 = [];
         if(page && false && page.viewer.catalog)
         {
            ...
            {
               _loc5_ = _loc4_.avatarRenderManager.petDataManager.getPetData(this.var_573,this);
               if(_loc5_)
               {
                  this.var_428 = _loc5_.colors;
         ...
         this._offers.add(this.var_573,_loc3_);
         events.addEventListener(WidgetEvent.CWE_SELECT_PRODUCT,this.onSelectProduct);
         events.addEventListener(WidgetEvent.CWE_COLOUR_INDEX,this.onColourIndex);
         events.addEventListener(WidgetEvent.const_918,this.onApproveNameResult);
      ...
      private function onWidgetsInitialized(param1:CatalogWidgetsInitializedEvent = null) : void
      {
         ...
         var _loc3_:* = [];
         for each(_loc4_ in this.var_428)
         {
      ...
      private function onColourIndex(param1:CatalogWidgetColourIndexEvent) : void
      {
         ...
         this.var_604 = param1.index;
         if(this.var_604 < 0 || this.var_604 > this.var_428.length)
         {
            this.var_604 = 0;
         }
         this.updateImage();
      ...
      {
         if(param1.productCode == this.var_2854)
         {
      ...
      private function parseSellableBreeds(param1:Array) : Array
      {
         ...
         {
            if(_loc3_.type == this.var_573 && _loc3_.sellable)
            {
      ...
      private function updateBreedSelections() : void
      {
         ...
         while(_loc3_ < this.var_235.length)
         {
            ...
            _loc2_.push(_loc1_.getKey(this.getRaceLocalizationKey(this.var_573,_loc5_),this.getRaceLocalizationKey(this.var_573,_loc5_)));
            _loc3_++;
         }
      ...
      private function updateImage() : void
      {
         ...
         if(this.var_235 == null || this.var_929 >= this.var_235.length)
         {
            return;
         }
         var _loc2_:* = 0;
         if(this.var_604 >= 0 && this.var_604 < this.var_428.length)
         {
            _loc7_ = this.var_428[this.var_604] as IPetColor;
            if(_loc7_ != null)
            {
               _loc2_ = uint(_loc7_.rgb);
            }
         }
         var _loc3_:int = this.var_235[this.var_929] as int;
         var _loc4_:IAvatarImage = (page.viewer.catalog as HabboCatalog).avatarRenderManager.createPetImage(this.var_573,_loc3_,_loc2_,AvatarScaleType.const_61,this);
         if(_loc4_ == null)
         {
            return;
         }
         _loc4_.initActionAppends();
         _loc4_.appendAction(AvatarAction.const_340,AvatarAction.const_1006);
      ...
      private function getPurchaseParameters() : String
      {
         ...
         if(this.var_604 >= this.var_428.length)
         {
            return "";
         }
         var petColor:IPetColor = this.var_428[this.var_604] as IPetColor;
         var color:uint = petColor.rgb;
         var breed:int = this.var_235[this.var_929] as int;
