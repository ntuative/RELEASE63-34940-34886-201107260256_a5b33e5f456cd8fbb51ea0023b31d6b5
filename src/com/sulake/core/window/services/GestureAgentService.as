package com.sulake.core.window.services
{
   ...
   public class GestureAgentService implements IGestureAgentService, IDisposable
   {
      ...
      public function begin(param1:IWindow, param2:Function, param3:uint, param4:int, param5:int) : IWindow
      {
         ...
         {
            this._window = param1;
            this._window.addEventListener(WindowEvent.const_368,this.clientWindowDestroyed);
            this.var_313 = param2;
            this._working = true;
            this.var_1432 = param4;
            this.var_1431 = param5;
            this.var_236 = new Timer(40,0);
            this.var_236.addEventListener(TimerEvent.TIMER,this.operate);
            this.var_236.start();
         }
         return _loc6_;
      }
      ...
      protected function operate(param1:TimerEvent) : void
      {
         this.var_1432 *= 0.75;
         this.var_1431 *= 0.75;
         if(Math.abs(this.var_1432) <= 1 && Math.abs(this.var_1431) <= 1)
         {
            this.end(this._window);
         }
         else if(this.var_313 != null)
         {
            this.var_313(this.var_1432,this.var_1431);
         }
      }
      ...
      public function end(param1:IWindow) : IWindow
      {
         ...
         {
            this.var_236.stop();
            this.var_236.removeEventListener(TimerEvent.TIMER,this.operate);
            this.var_236 = null;
         }
         if(this._working)
         {
            if(this._window == param1)
            {
               ...
               {
                  this._window.removeEventListener(WindowEvent.const_368,this.clientWindowDestroyed);
               }
               this._window = null;
               this._working = false;
            }
